...
/**
 * Snapshot profile stats (one per platform per day).
 * @param {"instagram"|"facebook"} platform
 * @param {object} profile — from getInstagramProfile() or getFacebookProfile()
 */
export async function persistProfileSnapshot(platform, profile) {
  ...
  if (error) {
    console.warn(`[social-persist] ${platform} profile snapshot failed:`, error.message);
  }
}
