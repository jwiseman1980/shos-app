...
export async function createFamilyContact({
  ...
  if (email) {
    const existing = await sfQuery(
      `SELECT Id, Name, Email, MailingStreet, MailingCity, MailingState, MailingPostalCode
       FROM Contact WHERE Email = '${email.replace(/'/g, "\\'")}' LIMIT 1`
    );
    if (existing.length > 0) {
      // Update address if provided and not already set
      if (mailingStreet && !existing[0].MailingStreet) {
        await sfUpdate("Contact", existing[0].Id, {
      ...
      return {
        ...
        wasExisting: true,
        address: {
          street: mailingStreet || existing[0].MailingStreet || "",
          city: mailingCity || existing[0].MailingCity || "",
          state: mailingState || existing[0].MailingState || "",
          postalCode: mailingPostalCode || existing[0].MailingPostalCode || "",
        },
      };
    }
  }
  ...
  if (mailingStreet) {
    contactFields.MailingStreet = mailingStreet;
    contactFields.MailingCity = mailingCity;
    contactFields.MailingState = mailingState;
    contactFields.MailingPostalCode = mailingPostalCode;
    contactFields.MailingCountry = mailingCountry;
  }

  const contact = await sfCreate("Contact", contactFields);
