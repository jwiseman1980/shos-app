...
/**
 * Triage all "needs_decision" order items:
 * - Items with existing designs -> advance to "ready_to_laser"
 * - Items without designs -> set to "design_needed"
 */
...
export async function createDonatedOrder({
  ...
  try {
    ...
    for (const { qty, size, suffix } of sizes) {
      ...
      const initialStatus = hasDesign ? "ready_to_laser" : "design_needed";
