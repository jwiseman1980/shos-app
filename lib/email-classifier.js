...
export function classifyEmail(from = "", subject = "") {
  ...
  if (/va\.gov|veteran|vfw|vba|vha/i.test(from) || /va\s+(health|benefit|appointment|medication)/i.test(subject)) return "VA-VET";
  if (/travismanion|manslegacy|legacies.*alive|heroes.*indeed|night.*stalker|memorial.*foundation/i.test(from)) return "PARTNER-ORG";
  if (/chase\.com|alliantcreditunion|freedommortgage|servicefinance|usaa/i.test(from) && /statement\s*(is\s*)?(ready|available|now available)|your\s.*statement/i.test(subject)) return "STATEMENT";
  if (/tldrnewsletter|newsletter|digest/i.test(from)) return "NEWSLETTER";
  if (/fort.*mill|zillow|ahrn|rental|lease|tenant|landlord/i.test(from) || /house.*rent|property.*tour|application.*rent/i.test(subject)) return "PROPERTY";
  return "OTHER";
}
...
export const CATEGORY_STYLES = {
  ...
  "VA-VET":               { label: "VA/Vet",    color: "#14b8a6" },
  "PARTNER-ORG":          { label: "Partner",   color: "#8b5cf6" },
  "NEWSLETTER":           { label: "News",      color: "#6b7280" },
