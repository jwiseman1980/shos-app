...
import { captureFamilyMessagesFromOrder } from "@/lib/data/messages";
...
export async function processIncomingOrder({
  ...
  // ── 3b. Capture tribute message into family_messages (best-effort) ─────
  // Squarespace's customerMessage field is purchasers' tribute to the fallen.
  // The legacy path was batch-only via migrate-sf-to-supabase; this captures
  // it in real time so the family-messages dashboard stays current.
  let familyMessagesResult = null;
  if (notes && notes.trim().length > 0) {
    try {
      familyMessagesResult = await captureFamilyMessagesFromOrder(sb, {
        orderNumber,
      ...
      console.warn("[processIncomingOrder] family_messages capture failed:", e.message);
      familyMessagesResult = { error: e.message };
    }
  }
  ...
  // Squarespace orders are imported automatically by ShipStation's native
  // store integration (storeId 1347853), so pushing them here would create
  // duplicates in the Manual Orders store. Only push for non-Squarespace
  // sources (donated, wholesale, manual, Stripe, etc.).
  ...
  if (source !== "squarespace") {
    ...
    shipStationResult = { skipped: "squarespace orders flow through native ShipStation integration" };
  }

  // ── 6. Slack notification ───────────────────────────────────────────────
  // Routine paid Squarespace sales already have ShipStation/email confirmations,
  // so silence Slack for them. Donated/wholesale/gift/replacement orders and
  // non-Squarespace sources still post — those need operator eyes.
  const isRoutinePaidSale = source === "squarespace" && (!orderType || orderType === "paid");
  const slackWebhook = process.env.SLACK_SOP_WEBHOOK;
  if (slackWebhook && !isRoutinePaidSale) {
    try {
      ...
      const ssNote = shipStationResult?.error
        ...
        : shipStationResult?.skipped
        ? " (ShipStation native import)"
        : " ✓ ShipStation";
