...
export async function createTask({ title, description, priority = "medium", role, assigned_to, due_date, domain, hero_id, organization_id, sop_ref, tags, source_type, source_id }) {
  const supabase = getServerClient();
  const { data, error } = await supabase.from("tasks").insert({
    ...
    source_id,
    status: "todo",
  }).select("id, title, priority, role, due_date, assigned_to").single();

  if (error) return `Failed to create task: ${error.message}`;

  // Slack DM the assignee if one was set at creation time
  if (assigned_to) {
    try {
      let email = null;
      if (typeof assigned_to === "string" && assigned_to.includes("@")) {
        email = assigned_to;
      } else {
        const { data: u } = await supabase
          .from("users")
          .select("email")
          .or(`id.eq.${assigned_to},name.ilike.${assigned_to}`)
          .limit(1)
          .single();
        email = u?.email || null;
        if (!email) {
          try {
            ...
            const vol = vols.find((v) => v.name.toLowerCase() === String(assigned_to).toLowerCase());
            email = vol?.email || null;
          } catch {}
        }
      }
      if (email) {
        const { sendSlackDm, buildTaskAssignedMessage } = await import("@/lib/slack-actions");
        await sendSlackDm(
          email,
      ...
      console.warn("[createTask] Slack DM failed:", err.message);
    }
  }

  return `Task created: "${title}" [${priority}] assigned to ${role || "unassigned"}${due_date ? ` due ${due_date}` : ""}${assigned_to ? ` (assigned_to: ${assigned_to})` : ""} (id: ${data.id})`;
}
