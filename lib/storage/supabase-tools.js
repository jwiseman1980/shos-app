...
export async function updateTask({ task_id, status, priority, assigned_to, notes }) {
  ...
  const { data, error } = await supabase
    ...
    .update(updates)
    .eq("id", task_id)
    .select("id, title, status, priority, role, assigned_to, created_at")
    .single();

  if (error) return `Failed to update task: ${error.message}`;

  // Log accomplishment when task is marked done
  if (status === "done" && data) {
    logExecution({
      userName: data.assigned_to || "Joseph",
      itemType: "task",
      itemId: data.id,
      title: data.title,
      description: `Task completed: ${data.title}`,
      domain: data.role || "operator",
