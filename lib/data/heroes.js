...
async function fetchHeroesFromSupabase({ activeOnly = true } = {}) {
  const sb = getServerClient();
  let query = sb
    .from("heroes")
    .select(`
      *,
      family_contact:contacts_legacy!family_contact_id(id, first_name, last_name, email, relationship),
  ...
  if (activeOnly) {
    query = query.eq("active_listing", true);
  }
  ...
  return (data || []).map((r) => ({
    ...
    familyContactRelationship: Array.isArray(r.family_contact?.relationship)
      ? r.family_contact.relationship.join(", ") || null
      : r.family_contact?.relationship || null,
...
async function loadHeroes({ activeOnly = true } = {}) {
  try {
    return await fetchHeroesFromSupabase({ activeOnly });
  } catch (err) {
    ...
    return activeOnly ? heroData.filter((h) => h.activeListing) : heroData;
  }
}
...
export async function getHeroes() {
  return loadHeroes({ activeOnly: true });
}

export async function getAllHeroes() {
  return loadHeroes({ activeOnly: false });
}

export async function getHeroById(id) {
  // Lookups by id may target archived/inactive heroes (e.g. detail pages).
  const heroes = await loadHeroes({ activeOnly: false });
...
/**
 * All individual-hero anniversaries falling within the next `days` days
 * (inclusive of today). Sorted ascending by days-until.
 */
export async function getAnniversariesNext(days = 30) {
  const { daysUntilAnniversary } = await import("@/lib/dates");
  const heroes = await getHeroes();
  return heroes
    .filter((h) => h.memorialType === "individual" && h.anniversaryMonth && h.anniversaryDay)
    .map((h) => ({
      ...h,
      daysUntil: daysUntilAnniversary(h.anniversaryMonth, h.anniversaryDay),
    }))
    .filter((h) => h.daysUntil != null && h.daysUntil <= days)
