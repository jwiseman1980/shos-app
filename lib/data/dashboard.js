...
export async function loadQueueItems(user) {
  ...
  const [
    ...
    // Operational tasks — exclude dev/build domain tasks which are internal build work
    sb.from("tasks")
      .select("*")
      .in("status", ["backlog", "todo", "in_progress"])
      .not("domain", "in", '("dev","build","infra")')
  ...
  if (tasksResult.data) {
    for (const task of tasksResult.data) {
      // Only show tasks assigned to this user (or unassigned)
      if (task.assigned_to && task.assigned_to !== user?.name && task.assigned_to !== user?.email) continue;
  ...
  const myAnniversaries = anniversariesResult.filter((h) => {
    ...
    return !["email_sent", "sent", "complete", "completed", "skipped", "scheduled"].includes(status);
  });

  for (const hero of myAnniversaries) {
    // Only show anniversaries assigned to this user (or unassigned)
    const assignee = (hero.anniversaryAssignedTo || "").trim().toLowerCase();
    const userName = (user?.name || "").trim().toLowerCase();
    const userEmail = (user?.email || "").trim().toLowerCase();
    if (assignee && assignee !== userName && assignee !== userEmail) continue;
    ...
    let heroYears = null;
    if (hero.memorialDate) {
      const mDate = new Date(hero.memorialDate + "T12:00:00");
      const now = new Date();
      heroYears = now.getFullYear() - mDate.getFullYear();
      if (now < new Date(now.getFullYear(), mDate.getMonth(), mDate.getDate())) heroYears--;
    }
    ...
    items.push({
      ...
      heroSfId: hero.sfId,
      // Enriched hero data for AnniversaryDetail component
      heroName: displayName,
      heroBranch: hero.branch,
      heroMemorialDate: hero.memorialDate,
      heroYears,
      familyContactName: hero.familyContactName,
      familyContactEmail: hero.familyContactEmail,
      familyContactRelationship: hero.familyContactRelationship,
      anniversaryStatus: hero.anniversaryStatus,
      anniversaryNotes: hero.anniversaryNotes,
      heroSku: hero.sku,
    });
  }
...
export async function loadScoreboardStats(user) {
  ...
  const myAnniversaries = isAdmin
    ? anniversariesThisMonth
    : anniversariesThisMonth.filter((h) => {
        const a = (h.anniversaryAssignedTo || "").trim().toLowerCase();
        return a === (user?.name || "").trim().toLowerCase() || a === (user?.email || "").trim().toLowerCase();
      });
...
export function mergeCalendarAndQueue(calendarEvents, queueItems) {
  ...
  for (const item of queueItems) {
    ...
    merged.push({
      // Spread all item fields (includes enriched anniversary data)
      ...item,
      // Override/normalize standard fields
  ...
  // Normalize: overdue items get today's date (they're actionable today, not in the past)
  for (const item of merged) {
    if (item.date && item.date < today) item.date = today;
  }

  merged.sort((a, b) => {
    // Today first, then future
    if (a.date !== b.date) return a.date < b.date ? -1 : 1;
    // Time-blocked calendar items always sort first (by time)
