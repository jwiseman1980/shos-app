...
export async function getOrderStats() {
  try {
    ...
    // Single query — fetch all status values and aggregate in JS.
    // Avoids 6 serial round-trips and catches any status values outside the expected enum.
    const [statusResult, paidResult, donatedResult] = await Promise.all([
      sb.from("order_items").select("production_status"),
    ...
    for (const row of statusResult.data || []) {
      const s = row.production_status;
      if (s) counts[s] = (counts[s] || 0) + 1;
    }

    const ns = counts["not_started"] || 0;
    const dn = counts["design_needed"] || 0;
    const rtl = counts["ready_to_laser"] || 0;
    const ip = counts["in_production"] || 0;
    const rts = counts["ready_to_ship"] || 0;

    return {
      notStarted: ns,          // awaiting triage
      designNeeded: dn,         // confirmed needs design (Ryan's queue)
      designTotal: ns + dn,     // backward-compat: total pre-laser
      readyToLaser: rtl,
      inProduction: ip,
      readyToShip: rts,
      shipped: counts["shipped"] || 0,
      totalActive: ns + dn + rtl + ip + rts,
      totalPaid: paidResult.count || 0,
      totalDonated: donatedResult.count || 0,
      rawCounts: counts,        // expose raw data for debugging
    };
  } catch (err) {
    console.error("Order stats error:", err.message);
    return { notStarted: 0, designNeeded: 0, designTotal: 0, readyToLaser: 0, inProduction: 0, readyToShip: 0, shipped: 0, totalActive: 0, totalPaid: 0, totalDonated: 0, rawCounts: {} };
  }
}
...
/**
 * Get comprehensive historical stats: total sold, donated, revenue, this month/year.
 */
export async function getHistoricalStats() {
  try {
    ...
    const [paidResult, donatedResult] = await Promise.all([
      sb
        .from("order_items")
        .select("quantity, unit_price, order:orders!order_id(order_type, order_date)")
        .eq("order.order_type", "paid"),
      sb
        .from("order_items")
        .select("quantity, order:orders!order_id(order_type, order_date)")
        .eq("order.order_type", "donated"),
    ]);

    const paidItems = paidResult.data || [];
    const donatedItems = donatedResult.data || [];

    const totalSold = paidItems.reduce((s, i) => s + (i.quantity || 1), 0);
    const totalDonated = donatedItems.reduce((s, i) => s + (i.quantity || 1), 0);
    const totalRevenue = paidItems.reduce((s, i) => s + (i.unit_price || 0) * (i.quantity || 1), 0);

    const soldThisYear = paidItems
      .filter((i) => i.order?.order_date >= yearStart)
      .reduce((s, i) => s + (i.quantity || 1), 0);
    const soldThisMonth = paidItems
      .filter((i) => i.order?.order_date >= monthStart)
      .reduce((s, i) => s + (i.quantity || 1), 0);
    const revenueThisYear = paidItems
      .filter((i) => i.order?.order_date >= yearStart)
    ...
    console.error("getHistoricalStats error:", err.message);
    return {
...
/**
 * Get all order items (paid + donated) for the full history table.
 */
export async function getAllOrderItems(limit = 750) {
  try {
    const sb = getServerClient();
    const { data: items, error } = await sb
      .from("order_items")
      .select(`
        ...
        order:orders!order_id(id, order_number, order_type, order_date, billing_name, billing_email, shipping_city, shipping_state),
    ...
    console.error("getAllOrderItems error:", err.message);
    return [];
  }
}

/**
 * Get top heroes by paid bracelet quantity.
 */
export async function getTopHeroesByOrders(limit = 12) {
  try {
    const sb = getServerClient();
    const { data: items, error } = await sb
      .from("order_items")
    ...
    for (const item of items || []) {
      ...
      const heroName = item.hero?.name;
      if (!heroId || !heroName) continue;
      if (!heroMap.has(heroId)) {
        heroMap.set(heroId, { heroId, heroName, totalQty: 0, lineCount: 0 });
      }
      const entry = heroMap.get(heroId);
    ...
    console.error("getTopHeroesByOrders error:", err.message);
    return [];
  }
}

/**
 * Get bracelet counts + revenue broken down by order_type.
 * Returns an array of { type, orderCount, itemCount, braceletCount, revenue }
 * sorted by bracelet volume descending.
 */
export async function getOrderTypeSummary() {
  try {
    const sb = getServerClient();

    const ORDER_TYPES = ["paid", "donated", "wholesale", "gift", "replacement"];

    const results = await Promise.all(
      ORDER_TYPES.map(async (t) => {
        const [ordersRes, itemsRes] = await Promise.all([
          ...
          sb
            .from("order_items")
            .select("quantity, unit_price, order:orders!order_id(order_type)")
            .eq("order.order_type", t),
        ]);

        const items = itemsRes.data || [];
        const braceletCount = items.reduce((s, i) => s + (i.quantity || 1), 0);
    ...
    return results.filter((r) => r.orderCount > 0 || r.braceletCount > 0);
  } catch (err) {
    console.error("getOrderTypeSummary error:", err.message);
    return [];
  }
}
