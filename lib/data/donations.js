...
// ---------------------------------------------------------------------------
// Create — manual donation entry (for Donorbox, Stripe, PayPal, checks, events)
// ---------------------------------------------------------------------------

/** Create a new Donation__c record in Salesforce */
export async function createDonation(data) {
  if (!useSalesforce) return { success: false, error: "SF_LIVE is not enabled" };

  try {
    const { sfCreate } = await import("@/lib/salesforce");
    const fields = {
      Donor_First_Name__c: data.donorFirstName || null,
      Donor_Last_Name__c: data.donorLastName || null,
      Donor_Email__c: data.donorEmail || null,
      Donation_Amount__c: data.amount,
      Donation_Date__c: data.donationDate,
      Source__c: data.source || null,
      Payment_Method__c: data.paymentMethod || null,
    };

    // New finance fields — may not exist in SF yet, wrapped in try
    try {
      if (data.donationType) fields.Donation_Type__c = data.donationType;
      if (data.restricted !== undefined) fields.Restricted__c = data.restricted;
      if (data.designation) fields.Designation__c = data.designation;
      if (data.receiptStatus) fields.Receipt_Status__c = data.receiptStatus;
    } catch {
      // Fields may not exist yet in SF — that's okay, base fields still save
    }

    const result = await sfCreate("Donation__c", fields);
