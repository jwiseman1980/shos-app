...
/** @returns {Promise<object[]>} all heroes */
...
/** @returns {Promise<object[]>} only heroes with activeListing === true */
export async function getActiveHeroes() {
  const heroes = await loadHeroes();
  return heroes.filter((h) => h.activeListing);
}

/** @returns {Promise<object[]>} heroes whose anniversaryMonth matches */
export async function getAnniversariesByMonth(month) {
  const heroes = await loadHeroes();
  return heroes.filter((h) => h.anniversaryMonth === month);
}

/** @returns {Promise<object[]>} heroes whose anniversary is this month */
export async function getAnniversariesThisMonth() {
  return getAnniversariesByMonth(getCurrentMonth());
}

/** @returns {Promise<object>} aggregate stats about the hero dataset */
export async function getHeroStats() {
  const heroes = await loadHeroes();
  const total = heroes.length;
  const active = heroes.filter((h) => h.activeListing).length;
