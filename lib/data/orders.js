...
export async function triageNeedsDecision() {
  try {
    ...
    const { data: items, error } = await sb
      .from("order_items")
      .select(`
        id, name, lineitem_sku, memorial_bracelet_id,
        heroes:memorial_bracelet_id(has_graphic_design, bracelet_design_created)
      `)
    ...
    for (const item of items) {
      ...
      if (designCheck.hasDesign) {
        try {
          ...
          if (!item.memorial_bracelet_id) {
            updateFields.memorial_bracelet_id = designCheck.heroId;
          }
    ...
    const { data: items, error } = await sb
      ...
      .select(`
        *,
        ...
        hero:heroes!memorial_bracelet_id(has_graphic_design, bracelet_design_created, design_brief, design_brief_6in)
      `)
  ...
  try {
    ...
    for (const { qty, size, suffix } of sizes) {
      ...
      if (designCheck?.heroId) {
        itemFields.memorial_bracelet_id = designCheck.heroId;
      }
...
function mapLineItem(r) {
  ...
  return {
    ...
    memorialBraceletId: r.memorial_bracelet_id || "",
...
export async function getOrdersByMonth(month, year) {
  try {
    ...
    const { data: items, error } = await sb
      ...
      .select(`
        *,
        hero:heroes!memorial_bracelet_id(name, organization:organizations!organization_id(id, name)),
