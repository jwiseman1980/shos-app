    ...
    const items = await sfQuery(
      `SELECT Id, Name, Lineitem_sku__c, Quantity__c, Unit_Price__c,
       ...
       ,Memorial_Bracelet__r.Has_Graphic_Design__c,
              ...
              Memorial_Bracelet__r.Design_Brief_6in__c
       FROM Squarespace_Order_Item__c
...
export async function createDonatedOrder({
  ...
  try {
    ...
    if (shippingName || shippingAddress1) {
      orderFields.Shipping_Name__c = shippingName || recipientName;
      orderFields.Shipping_Address1__c = shippingAddress1;
      orderFields.Shipping_City__c = shippingCity;
      orderFields.Shipping_State__c = shippingState;
      orderFields.Shipping_Postal__c = shippingPostal;
      orderFields.Shipping_Country__c = shippingCountry;
    }

    const order = await sfCreate("Squarespace_Order__c", orderFields);
    ...
    for (const { qty, size, suffix } of sizes) {
      ...
      const itemFields = {
        ...
        Lineitem_sku__c: itemSku,
        Quantity__c: qty,
        Unit_Price__c: 0,
        Bracelet_Size__c: size,
...
function mapLineItem(r) {
  ...
  return {
    ...
    // Prefer /api/designs/download proxy (SKU-aware, pulls directly from SF Files)
    // Falls back to Design_Brief field URL if no SF file exists
    ...
    designUrl: (() => {
      ...
      const hasFile = r.Memorial_Bracelet__r?.Has_Graphic_Design__c || r.Memorial_Bracelet__r?.Bracelet_Design_Created__c;
      if (sku && hasFile) return `/api/designs/download?sku=${encodeURIComponent(sku)}`;
      const is6in = /-6D?$/i.test(sku);
      const brief = is6in
        ? (r.Memorial_Bracelet__r?.Design_Brief_6in__c || r.Memorial_Bracelet__r?.Design_Brief__c || "")
        : (r.Memorial_Bracelet__r?.Design_Brief__c || "");
